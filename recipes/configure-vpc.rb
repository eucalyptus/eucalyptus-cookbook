...
bgp_peers.each do |bgp_info|
  bash "Doing BGP entry for: router=#{bgp_info['router-name']}  port-ip=#{bgp_info['port-ip']}" do
    code <<-EOH
      ROUTER_ID=$(#{midonet_command_prefix} list router name #{bgp_info['router-name']} | awk '{print $2}')
      PORT_ID=$(#{midonet_command_prefix} router $ROUTER_ID list port address #{bgp_info['port-ip']} | awk '{print $2}')

      CHECK_ASN="#{midonet_command_prefix} list router name #{bgp_info['router-name']} | grep #{bgp_info['local-as']}"
      SET_ASN="#{midonet_command_prefix} router $ROUTER_ID set asn #{bgp_info['local-as']}"
      ...
      CHECK_BGP_PEER=$(#{midonet_command_prefix} router $ROUTER_ID bgp-peer list)
      if [[ ! -z $CHECK_BGP_PEER ]]; then
        echo "BGP Peer already configured."
        BGP_PEER_ID=$(#{midonet_command_prefix} router $ROUTER_ID bgp-peer list | awk '{print $2}')
      else
        BGP_PEER_ID=$(#{midonet_command_prefix} router $ROUTER_ID add bgp-peer asn #{bgp_info['remote-as']} address #{bgp_info['peer-address']})
        echo "Setting up BGP Peer."
      fi

      CHECK_BGP_NETWORK="#{midonet_command_prefix} router $ROUTER_ID bgp-network list | grep #{bgp_info['route']}"
      ADD_BGP_NETWORK="#{midonet_command_prefix} router $ROUTER_ID add bgp-network net #{bgp_info['route']}"
